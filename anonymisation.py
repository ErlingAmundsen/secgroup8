...
def bin_age(data_private):
    ...
    data_private["dob"] = data_private["dob"].apply(
        lambda x: current_year - x.year
...
def bin_citezenship(data_private):
    data_private["citizenship"] = data_private["citizenship"].apply(
        lambda x: "Other" if x != "Denmark" else x
    )
    return data_private
...
def anonymize(data_private: pd.DataFrame):
    ...
    if "education" in data_private.columns:
        original_re_identification_columns = [
            "name",
            "sex",
            "dob",
            "zip",
    ...
    original_k_anonymity_groups = data_private.groupby(
        original_re_identification_columns
    ).size()
