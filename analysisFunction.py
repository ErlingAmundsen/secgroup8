...
def K_anonymity(dataPrivate: pd.DataFrame):
    ...
    print(
        dataPrivate.groupby(col)
        .size()
        .where(dataPrivate.groupby(col).size() == 1)
        .dropna()
    )
    return k_anym
...
def run_analysis_a(
    dataPrivate: pd.DataFrame,
    dataPublic: pd.DataFrame,
    ...
    evotesRed = (
        dataPrivate["party"][dataPrivate["evote"] == 1].value_counts()["Red"]
        / totalVotesElec
    )
    ...
    votesRed = (
        dataPrivate["party"][dataPrivate["evote"] == 0].value_counts()["Red"]
        / totalVotesNorm
    )
...
def create_graph_b(
    ...
    for t in targets:
        selectedItems.append(
            dataPrivate[dataPrivate[target] == t]
            .groupby(group)
            .size()
    ...
    if colors:
        ...
        df.plot.bar(rot=rot, title=title, ax=ax)
...
def run_analysis_b(dataPrivate: pd.DataFrame):
    ...
    create_graph_b(
        ...
        b_list,
        ["red", "green", "orange"],
        title="voting dist based on sex",
        ax=axes[0],
    )
    create_graph_b(
        dataPrivate,
        ...
        b_list,
        ["red", "green", "orange"],
        title="voting dist based on education",
        ax=axes[1],
    )
    create_graph_b(
        dataPrivate,
        ...
        b_list,
        ["red", "green", "orange"],
        title="voting dist based on age",
        ax=axes[2],
    )
...
def main():
    ...
    data, plt = run_analysis_a(
        data_private, data_public, data_results, "Red votes non-anonymised"
    )
    # plt.show()
